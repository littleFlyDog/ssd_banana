...
def eval_fn(model, valid_iter, device):
    model.eval()
    cls_correct_sum, bbox_mae_sum, total_positives, total_samples = 0.0, 0.0, 0, 0
    with torch.no_grad():
        for features, label in valid_iter:
            ...
            if num_positives_in_batch > 0:
                ...
                cls_correct_sum += (preds_on_fg.argmax(dim=-1) == labels_on_fg).sum().item()
...
def train_fn(num_epochs, model, train_iter, valid_iter,trainer, calc_loss, device):    
    ...
    for epoch in range(num_epochs):
        ...
        avg_cls_acc, avg_bbox_mae = eval_fn(model, valid_iter, device)
        if avg_cls_acc>=best_acc:
            ...
            if patience==0:
                ...
                break
        writer.add_scalars("LOSS",{'train_loss':avg_loss,'valid_cls_acc':avg_cls_acc,'avg_bbox_mae':avg_bbox_mae},epoch)
